...
@implementation UIView (AliStock)
...
- (UIImage *)asSnapshotImageWithFrame:(CGRect)frame shouldTryiOS7Method:(BOOL)shouldTryiOS7Method
{
        ...
        )
    {
        ...
        if (!captured)
            NSLog(@"capture may have failed with view %@, %@", self, NSStringFromCGRect(frame));
    }
    else
    {
...
- (BOOL)isViewVisible
{
    if (self && self.window && !self.hidden && self.alpha > 0) {
        return YES;
    }
...
@implementation UITableViewCell (AliStock)

- (UITableView *)afwTableView
{
    return (UITableView *)[self asFindParent:^BOOL(UIView *view, UIView *parent) {
        return [parent isKindOfClass:[UITableView  class]];
    }];
}

- (void)afwReload
{
    UITableView *tableView = [self afwTableView];
